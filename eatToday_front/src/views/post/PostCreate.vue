...
export default {
  ...
  methods: {
    ...
    async submit() {
      ...
      const contentImageDataUrls = await Promise.all(
        this.contentImages.map((f) => toDataURL(f))
      );
