...
export default {
  ...
  data() {
    return {
      ...
      loadingMore: false,
      // ✅ 로컬 기본 프사
      defaultAvatar: '/image/user-profile/basic_profile.jpg',
      fallbackThumb: '/images/placeholder-thumb.jpg',
    }
  },
  computed: {
    alcoholNo() {
    ...
    copy() {
      ...
      if (isWine) {
        return {
          ...
          typesTitle: '🍷 와인의 종류',
          types: [
            { term: '레드 와인', desc: '짙은 과실 향과 탄닌이 주는 구조감이 특징. 스테이크 같은 육류와 훌륭한 궁합을 보여요.', muted: '대표 품종: 카베르네 소비뇽, 메를로, 피노 누아' },
            { term: '화이트 와인', desc: '상큼한 산미와 깔끔한 피니시. 해산물·치킨·샐러드와 잘 어울립니다.', muted: '대표 품종: 샤르도네, 소비뇽 블랑, 리슬링' },
            { term: '스파클링 와인', desc: '기포가 선사하는 청량감과 미세한 질감. 전채요리부터 브런치까지 두루 활용돼요.', muted: '대표: 샴페인, 프로세코, 까바' },
            { term: '로제 와인', desc: '레드의 향과 화이트의 상쾌함 사이. 가벼운 meat & spice 메뉴에도 유연해요.', muted: null },
          ],
      ...
      if (isSoju) {
        return {
          ...
          typesTitle: '🍶 소주의 종류',
          types: [
            { term: '증류식 소주', desc: '전통 방식 증류. 깊고 풍부한 향, 부드러운 목넘김.', muted: '대표 브랜드: 화요, 일품진로, 대장부' },
            { term: '희석식 소주', desc: '가볍고 깨끗. 누구나 편하게 즐기기 좋아요.', muted: null },
          ],
          pairTitle: '🍽 소주 × 안주, 완벽한 페어링',
          pairs: [
            '<strong>매운맛</strong> — 불닭발, 오돌뼈, 순대볶음',
            '<strong>기름진 요리</strong> — 삼겹살, 족발, 제육볶음',
            '<strong>담백한 해산물</strong> — 문어숙회, 오징어볶음',
          ],
          tip: 'TIP: 차게 마시면 청량감↑, 온잔에서는 은은한 단맛이 잘 느껴져요.',
        }
      }
      return {
  ...
  methods: {
    async load(page = 0) {
      ...
      // 노멀라이즈된 모델을 그대로 사용 (avatar 이미 정규화됨)
      const normalize = (dto) => ({
        id: dto.id,
        title: dto.title,
        ...
        likes: dto.likes || 0,
        views: dto.views || 0,
        comment: dto.comment || 0,
        author: dto.author,
        avatar: dto.avatar, // ✅ 여기!
      })
